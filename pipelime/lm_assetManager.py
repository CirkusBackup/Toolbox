...
from lio.io_publishModel import IO_publishModel
...
def removeProject():
    # read current project from dropdown menu
...
def newAsset():
    ...
    if newAssetName:
        ...
        itemsText = []
        items = []
        for index in range(assetManagerUIWindow.mainWidget.listWidget_assetType.count()):
        ...
        itm = QtWidgets.QListWidgetItem(QtGui.QIcon(f'{dirname}/icons/lightRed.png'), newAssetName)
        assetManagerUIWindow.mainWidget.listWidget_assets.addItem(itm)
        for index in range(assetManagerUIWindow.mainWidget.listWidget_assets.count()):
...
def referenceAsset(assetName, imp):
    ...
    for item in assetName:
        ...
        try:
            ...
            print(f'unable to load reference {assetPath}')
...
def setAsset():
    ...
    try:
        assetName = assetManagerUIWindow.mainWidget.listWidget_assets.currentItem().text()
        assetType = assetManagerUIWindow.mainWidget.listWidget_assetType.currentItem().text()
        thumbPath = f'{projectsDictGlobal[project]["projectPath"]}/scenes/REF/{assetType}/{assetName}/.data/{assetName}_thumb.jpg'
    except Exception as e:
        # Don't continue if there is an error in getting the data
        print('Failed to get asset info: ' + str(e))
        return

    # find and display date
    try:
        refPath = f'{projectsDictGlobal[project]["projectPath"]}/scenes/REF/{assetType}/{assetName}_REF.mb'
    ...
    try:
        namePath = '%s/scenes/REF/%s/%s/.data/%s_REF.json' % (
            projectsDictGlobal[project]["projectPath"], assetType, assetName, assetName)
...
def setProject():
    projectsDict = IO.loadDictionary(f'{qtBase.local_path()}/projects.json')
    print(projectsDict)
    key = assetManagerUIWindow.mainWidget.project_comboBox.currentText()
    projectPath = projectsDict[key]["projectPath"]

    assetTypeFolders = []
    path = f'{projectPath}/scenes/REF/'
...
def assetManagerUI():
    ...
    window.mainWidget.pushButton_reference.clicked.connect(
        lambda: referenceAsset(window.mainWidget.listWidget_assets.selectedItems(), 0))
    # import button
    window.mainWidget.pushButton_import.clicked.connect(
        lambda: referenceAsset(window.mainWidget.listWidget_assets.selectedItems(), 1))
    # new button
    window.mainWidget.pushButton_new.clicked.connect(newAsset)
    # edit button
    window.mainWidget.pushButton_edit.clicked.connect(
        lambda: editAsset(window.mainWidget.listWidget_assets.currentItem().text()))
    # thumbnail button
    window.mainWidget.pushButton_thumb.clicked.connect(
        lambda: takeScreenshot(window.mainWidget.listWidget_assets.currentItem().text()))
    # save button

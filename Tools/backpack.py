...
# Reload existing instances of the modules.
reload(myUtils)
reload(moveFiles)
...
def loadPrefs():
    ...
    prefFile = f'{prefPath}/toolboxPrefs.json'
    try:
...
def browseToFolder():
    ...
    if folder:
        ...
        prefPath = myUtils.paths.userPrefsPath()
        jsonFileName = f'{prefPath}/toolboxPrefs.json'
...
def browseToToken():
    ...
    if file:
        ...
        token = getDbToken()
        if token:
            allFolders = moveFiles.httpDropbox.listFiles(
                '/Projects/Backpack/Sorted', token)
            populateOptionMenu(allFolders)
...
def exportFile():
    ...
    if len(objects) > 0:
        ...
        if cmds.optionMenu("storageType", q=True, v=True) == 'Local Folder':
            newpath = f'{cmds.textField("localFolderpath", q=True, tx=True)}/{class2}'
            # export alembic
            exportAbc(objects, newpath, name)
            # copy textures
            moveFiles.localFiles.copyFiles(
                filePaths, '%s/%s' % (newpath, name))
            ...
            if token:
                ...
                moveFiles.httpDropbox.uploadFileToDB(
                    '%s/%s.abc' % (newpath, name), '/Projects/Backpack/Sorted/%s/%s.abc' % (class2, name), token)
                # copy textures
                tempLocations = moveFiles.localFiles.copyFiles(
                    filePaths, '%s' % (newpath))
                for t in tempLocations:
                    fileName = t.split('/')[-1]
                    moveFiles.httpDropbox.uploadFileToDB(
                        t, '/Projects/Backpack/Sorted/%s/%s/%s' % (class2, name, fileName), token)
            else:
...
def BackpackWindow():
    ...
    # setting menu
    settingsButton = cmds.iconTextButton(
        style='iconOnly', image1='gear.png', c='toggleVis()')
    ...
    reloadButton = cmds.iconTextButton(
        style='iconOnly', image1='refresh.png', c='setText(\'modelNameInputText\')')

    modelBtn1 = cmds.button("modelBtn1", l='Pocket',
                            h=50, w=200, c='exportFile()')

    # model class menu
    modelClass = cmds.optionMenu(
        "modelClass", w=50, changeCommand='UpdateMenus()')
    cmds.menuItem(label="Create New")
    cmds.menuItem(divider=True)

    modelClassInputText = cmds.textField("modelClassInputText", w=50, vis=False,
                                         cc='ReturnToOptionMenu("modelClassInputText","modelClass",cmds.textField("modelClassInputText",q=True,tx=True ))')
    if(cmds.optionMenu(modelClass, q=True, numberOfItems=True) > 2):
        cmds.optionMenu(modelClass, edit=True, sl=3)
...
def dockingBackpack():
    ...
    cmds.workspaceControl(
        WorkspaceName, initialHeight=500,
        initialWidth=320, uiScript='BackpackWindow()'
    )

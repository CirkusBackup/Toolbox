...
class AddRow():
    ...
    def makeNewRow(self):
        ...
        def makeFloatSlider(self):
            valueField = cmds.floatSliderGrp(
                'valueField', l='', field=True, v=float(self.attrValue))
            return valueField

        def makeTextField(self):
            valueField = cmds.textField(
                'valueField', w=50, text=self.attrValue)
            return valueField
        ...
        cmds.button(delBtn, e=True, c=f'cmds.deleteUI(\'{thisParent}\'')
...
def LoopboxWindow():
    ...
    cmds.button(l="Drop Attribute Here", h=30, dpc=dropAddRow,
                c="clickAddRow()", bgc=(0.3, 0.3, 0.3))
...
def dockingLoopbox():
    ...
    cmds.workspaceControl(
        ...
        initialWidth=320, uiScript='LoopboxWindow()'
    )

...
def userIDWindow(initials):
    ...
    textMessage = cmds.text(
        label='Hey, it\'s been over a week are these still your initials?')
    textLabel = cmds.text(label='Initials')
    nameText = cmds.textField('nameText', width=250, tx='Custom')

    btn1 = cmds.button(
        height=50, label='Update',
        c='updateUserPrefs(inputText());incrementalSaveUI();cmds.deleteUI(\'User ID\')'
    )
    btn2 = cmds.button(
        height=50, label='Close',
        c='cmds.deleteUI(\'User ID\')'
    )
...
def user_install(initials):
    ...
    cmds.workspaceControl(
        workspaceName, initialHeight=100,
        initialWidth=300, uiScript='userIDWindow(\''+initials+'\')'
    )
# user_install()

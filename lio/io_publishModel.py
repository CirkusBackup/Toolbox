...
def reconnectMatarials(connections):
    for c in connections:
        # make connections
        c = c.replace('"', '')
        try:
            cmds.connectAttr(c.split(',')[0], c.split(',')[1])
        except:
            pass
...
def makeRef(refName, publishString):
    ...
    # set outliner colour
    cmds.setAttr(f'{publishString}.useOutlinerColor', 1)
    cmds.setAttr(f'{publishString}.outlinerColorR', 0.25)
    cmds.setAttr(f'{publishString}.outlinerColorG', 0.8)
    cmds.setAttr(f'{publishString}.outlinerColorB', 0.25)
    # refresh outliner
    try:
    ...
    if os.path.isfile(pathName):
        # make backup folder
        backupFolder = f'{parentFolder}/{currentFolder}/backup'
        if not os.path.exists(backupFolder):
            os.makedirs(backupFolder)
        count = 1
        backupExists = os.path.isfile(f'{backupFolder}/{refFileName}{count:d}')
        while backupExists == 1:
            count += 1
            backupExists = os.path.isfile(f'{backupFolder}/{refFileName}{count:d}')
        backupName = f'{backupFolder}/{refFileName}{count:d}'
        copyfile(pathName, backupName)
    # export .mb REF
    cmds.file(pathName, force=True, type='mayaBinary', pr=True, es=True)
    # log
    logOutput = [pathName, scenePath, backupName]
    return logOutput
...
def makeAlembic(refName, publishString):
    try:
        ...
        additionalAttributes += [
            ...
            'rsDoSmoothSubdivision', 'rsMinTessellationLength', 'rsOutOfFrustumTessellationFactor',
            'rsEnableDisplacement', 'rsMaxDisplacement', 'rsDisplacementScale'
        ]
        for attr in additionalAttributes:
            additionalAttr += ' -attr %s' % (attr)
        command = dedent(f'''
        -frameRange 1 1{additionalAttr}
        -stripnamespaces
        -uvWrite
        -worldSpace
        -writeVisibility
...
def exportShaders(publishName, scenePath):
    ...
    for s, shape in enumerate(allDecendingShapes):
        ...
        if shadingGroups:
            ...
            for n, shadingGrp in enumerate(shadingGroups):
                ...
                for node in nodes:
                    ...
                    if node != "<done>":
                        connectedNodes = cmds.listConnections(
                            node, t='transform', plugs=True, c=True, d=False, s=True
                        )
            ...
            if s > 0:
                data += ','
            data += '\n        {\n        "IOID": "%s",\n        "materials": [%s]%s\n        }' % (
                ID[0], shadingGrpsString, controlData)

    data += '\n ]\n}'
    # write connections out to text file
    WriteToDB(f'{workspace}/renderData/alembicShaders/{publishName}/{publishName}/', data, 'w')
    ...
    # write all shaders to disk
    for material in allMaterials:
        # process namespaces
        materialFilename = material.replace(':', '_')
        # export material
        cmds.select(material, r=True, noExpand=True)
        cmds.file(f'{workspace}/renderData/alembicShaders/{publishName}/{publishName}_{materialFilename}.mb',
                  force=True, typ='mayaBinary', pr=True, es=True)

        # increment to log
        shaderCount += 1
    # reconnect rig to materials after export
    reconnectMatarials(allConections)
...
def publishModel():
    ...
    if len(sel) == 1:
        ...
        # TODO re-implement rig seperation during shader export.
        numberOfFiles += exportShaders(publishName, scenePath)

...
def publishCamera(abcFilename, cameraName):
    ...
    # published file name
    exportString = f' -root {cameraName}'
    ...
    for attr in additionalAttributes:
        additionalAttr += f' -attr {attr}'
    command = '-frameRange %d %d%s -ro -stripNamespaces -worldSpace -dataFormat ogawa%s -file \"%scache/alembic/%s%s.abc\"' % (
    startFrame, endFrame, additionalAttr, exportString, workspace, relativePath, abcFilename)
    # write to disk
...
def renameCamera(selectionCamera):
    ...
    newName = cmds.rename(selectionCamera, f'{shotName}_s{startFrame}_e{endFrame}')
    return newName
...
def makeCameraName():
    ...
    camFileName = f'{filename}_CAMERA'
    return camFileName

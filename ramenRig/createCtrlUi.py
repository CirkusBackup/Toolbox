...
def colourAtPoint(rampName, point):
    ...
    def channelGrad(channel, p):
        channelDict = {'R': 0, 'G': 1, 'B': 2}
        channelInt = channelDict[channel]

        cmds.optionVar(stringValue=['tempRChannelOptVar', f'{rampList[channelInt]},{rampList[3]},1'])

        positions = rampList[3::5]
        for i, c in enumerate(positions[1:]):
            cmds.optionVar(stringValueAppend=['tempRChannelOptVar', f'{rampList[channelInt::5][i + 1]},{c},1'])

        tempGrad = cmds.gradientControlNoAttr(h=90, m=False)
        cmds.gradientControlNoAttr(tempGrad, e=True, optionVar='tempRChannelOptVar')

        channelValue = cmds.gradientControlNoAttr(tempGrad, q=True, valueAtPoint=p)
...
def createBtn(shape):
    ...
    if sel and win.checkBox_selection.isChecked():
        parent = ''
        if len(sel) > 1:
        ...
        for i, o in enumerate(sel):
            ...
            ctrlFullName = f'{ctrlName}_CTRL{side}'
            ...
            if win.checkBox_group.isChecked():
                ctrlGrp = cmds.group(ctrl, n=f'{ctrl[0]}_GRP')
...
def setRampCol():
    ...
    cmds.gradientControlNoAttr('falloffCurve', e=True, currentKeyColorValue=col)
...
def createCTRL_ui():
    ...
    cmds.gradientControlNoAttr('falloffCurve', e=True, currentKey=1, clv=rgbColour,
                               cc='ramenRig.createCtrlUi.setColour()')

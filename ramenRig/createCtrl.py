...
class MakeCtrlCurve:
    ...
    def setColour(self, ctrl):
        if self.ctrlColour:
            ...
            for s in curveShapes:
                ...
                cmds.setAttr(f'{s}.overrideColorRGB', self.ctrlColour[0], self.ctrlColour[1], self.ctrlColour[2])
    ...
    def makeShape(self):
        ...
        if self.shape.lower() == 'square':
            ...
            cvs = [f'{ctrl[0]}.cv[1]', f'{ctrl[0]}.cv[10]', f'{ctrl[0]}.cv[7]', f'{ctrl[0]}.cv[4]']
            ...
            cmds.xform(ctrl, t=self.pos)
            # shape star
            cvs = [f'{ctrl[0]}.cv[0]', f'{ctrl[0]}.cv[2]', f'{ctrl[0]}.cv[4]', f'{ctrl[0]}.cv[6]']
            ...
            cmds.parent(cmds.listRelatives(children[0], s=True, pa=True),
                        ...
                        cmds.listRelatives(children[2], s=True, pa=True), ctrl[0], add=True, s=True)
            ...
            ctrl = cmds.curve(d=1, p=[(-0.5, 1, 0), (-0.5, 0, 0), (-1, 0, 0), (0, -1, 0), (1, 0, 0), (0.5, 0, 0.0),
                                      (0.5, 1, 0), (-0.5, 1, 0)], n=self.ctrlName)
            cmds.xform(ctrl, t=self.pos)
            ctrl = [ctrl]
